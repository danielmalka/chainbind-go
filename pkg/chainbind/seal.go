...
// unpredictable identifiers (PRD Story 1 AC-7).
const packageIDBytes = 16

// segmentAlg and segmentWrapAlg are the fixed algorithm identifiers this
// build stamps into every manifest segment entry (TECHSPEC-001 §6.6
// decisions 1, 7).
const (
...
// only (architecture invariant 10).
var (
	// ErrNoAudiences is returned when a SealRequest names no audiences. A
	// package with no segments would be a signed statement about
	// nothing (TASK-001-06 carry-over from TASK-001-02's QA review).
...
// no meaning on Name beyond using it as a map/manifest key (D-004);
// callers and profiles choose their own vocabulary.
...
// itself is a profile's job (TASK-001-09) and out of scope here.
type SealRequest struct {
	...
	// execution is checked, and later bound, against (D-013).
	IntentRef string
	...
	// fields the authority's policy needs (D-009). Building it from a
	// domain payload is a profile's job; the core forwards it opaquely.
	Projection any
	...
	// context (TECHSPEC-001 §6.3); Seal fills in package_id itself.
	TenantID    string
	Environment string
	...
	// and attached under bindings.Extra (D-004). Nil for core-only use.
...
// data-encryption key or encrypting any byte (PRD Story 2 AC-1): a denial
// surfaces the authority's own reason (AC-2, wrapped in ErrIntentDenied),
// and an unreachable authority fails closed with no package returned and no
// fallback (AC-3, architecture invariant 6) — the deliberate asymmetric
// counterpart of Verify's "indeterminate" outcome.
//
...
// package id are signed together as one unit (architecture invariant 2).
func Seal(ctx context.Context, req SealRequest, s Signer, w KeyWrapper, iv IntentVerifier) (*Package, error) {
	...
	// (architecture invariant 7): it is what intent_commitment binds to,
	// and it too is fetched before any ciphertext exists.
...
func sealSegment(ctx context.Context, w KeyWrapper, aadCtx AADContext, aud Audience, plaintext []byte) (Segment, SealedSegment, string, error) {
	...
	// produced (TECHSPEC-001 §6.1) — hashed before it is split into the
	// wire format's separate ciphertext and tag fields below.
...
// before the view is built, because issuer.kid sits inside the view
// (TECHSPEC-001 §6.4) and the signature must commit to the identity of the
// key that produced it. Sign is called exactly once, over exactly the bytes
// the package claims were signed.
